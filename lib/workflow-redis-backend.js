...
WorkflowRedisBackend.prototype.init = function (callback) {
    ...
    self.client.on('error', function (err) {
        self.log.error({err: err}, 'Redis client error');
        self.emit('error', err);
    });
...
//        workflow properties
// cb - f(err, workflow)
WorkflowRedisBackend.prototype.createWorkflow = function (workflow, meta, cb) {
    ...
    // Add the name to the wf_workflow_names set in order to be able to check
    //  with SISMEMBER wf_workflow_names workflow.name
...
WorkflowRedisBackend.prototype.getWorkflow = function (uuid, meta, callback) {
    if (typeof (meta) === 'function') {
        callback = meta;
        meta = {};
    }
...
//        workflow properties
// cb - f(err, boolean)
WorkflowRedisBackend.prototype.deleteWorkflow = function (workflow, meta, cb) {
    if (typeof (meta) === 'function') {
        cb = meta;
        meta = {};
    }
...
//        workflow properties
// cb - f(err, workflow)
WorkflowRedisBackend.prototype.updateWorkflow = function (workflow, meta, cb) {
    if (typeof (meta) === 'function') {
        cb = meta;
        meta = {};
    }
...
//        job properties
// callback - f(err, job)
WorkflowRedisBackend.prototype.createJob = function (job, meta, callback) {
...
//        job properties
// callback - f(err, job)
WorkflowRedisBackend.prototype.getJob = function (uuid, meta, callback) {
...
WorkflowRedisBackend.prototype.getJobProperty =
function (uuid, prop, meta, cb) {
    ...
    return self.client.hget('job:' + uuid, prop, function (err, val) {
        if (err) {
            self.log.error({err: err});
            return cb(new wf.BackendInternalError(err));
        } else {
...
WorkflowRedisBackend.prototype.validateJobTarget = function (job, callback) {
    ...
    return self.client.smembers(
        ...
        function (err, members) {
            ...
            // verify none of them has the same parameters than the job we're
            // trying to queue:
                    ...
                    if (err) {
                        ...
                        if (aJob.workflow_uuid === job.workflow_uuid &&
                          ...
                          JSON.stringify(job.params)) {
                            // Already got same target, now also same workflow
                            // and same params fail it
...
WorkflowRedisBackend.prototype.nextJob = function (index, callback) {
    ...
    return self.client.sort('wf_queued_jobs',
...
WorkflowRedisBackend.prototype.finishJob = function (job, callback) {
    ...
    return self.client.lrem('wf_running_jobs', 0, job.uuid,
        function (err, res) {
            if (err) {
                self.log.error({err: err});
                return callback(new wf.BackendInternalError(err));
...
// callback - f(err, job) callback will be called with error if something
//            fails, otherwise it'll return the updated job using getJob.
WorkflowRedisBackend.prototype.updateJob = function (job, meta, callback) {
    if (typeof (job) === 'undefined') {
        return callback(new wf.BackendInternalError(
    ...
    return self.client.hmset('job:' + job.uuid, job, function (err, res) {
        if (err) {
            self.log.error({err: err});
            return callback(new wf.BackendInternalError(err));
        }
    ...
    callback)
{
    ...
    return self.client.hset('job:' + uuid, prop, val, function (err, res) {
        if (err) {
            self.log.error({err: err});
            return callback(new wf.BackendInternalError(err));
        }
...
// - callback - f(err, jobs)
WorkflowRedisBackend.prototype.getJobs = function (params, callback) {
    var self = this,
        multi = self.client.multi(),
...
//        job info
// - callback - f(err)
WorkflowRedisBackend.prototype.addInfo = function (uuid, info, meta, cb) {
...
//        job info
// - callback - f(err, info)
WorkflowRedisBackend.prototype.getInfo = function (uuid, meta, callback) {
    if (typeof (meta) === 'function') {
        callback = meta;
        meta = {};
    }
