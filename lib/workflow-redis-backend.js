...
var WorkflowRedisBackend = module.exports = function (config) {
    ...
    function getWorkflow(uuid, meta, callback) {
        ...
        client.hgetall('workflow:' + uuid, function (err, workflow) {
            if (err) {
                ...
                if (workflow.oncancel) {
                    workflow.oncancel = JSON.parse(workflow.oncancel);
                }
                if (workflow.timeout) {
    ...
    function getJobProperty(uuid, prop, meta, cb) {
        ...
        var encoded_props = ['chain', 'chain_results', 'oncancel',
            'oncancel_results', 'onerror', 'onerror_results', 'params'];
    ...
    function cancelJob(uuid, callback) {
        if (typeof (uuid) === 'undefined') {
            return callback(new wf.BackendInternalError(
              'WorkflowRedisBackend.cancelJob uuid(String) required'));
        }
    ...
    function countJobs(callback) {
        ...
        return client.smembers('wf_jobs', function (err, res) {
            ...
            res.forEach(function (uuid) {
                multi.hmget('job:' + uuid, 'created_at', 'execution');
            });
            ...
            return multi.exec(function (err, replies) {
                ...
                replies.forEach(function (job, i, arr) {
                    if (isNaN(new Date(job[0]).getTime())) {
                        job[0] = Number(job[0]);
                    }

                    if (job[0] < yesterday) {
                        stats.all_time[job[1]] += 1;
                    } else if (job[0] > yesterday && job[0] < _2hr) {
                        stats.past_24h[job[1]] += 1;
                    } else if (job[0] > _2hr && job[0] < _1hr) {
                        stats.past_hour[job[1]] += 1;
                    } else {
                        stats.current[job[1]] += 1;
                    }
                });
